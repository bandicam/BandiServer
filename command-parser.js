...
const CAUTION_MESSAGE_LENGTH = 150;
...
function canTalk(user, room, connection, message) {
	...
	if (message) {
		...
		if (message.length > CAUTION_MESSAGE_LENGTH && !user.can('broadcast')) {
			if (!user.timelastlongmsg) return user.timelastlongmsg = Date.now();
			if (Date.now() - user.timelastlongmsg) < MESSAGE_COOLDOWN) {
				user.timelastlongmsg = Date.now();
				user.send('You have been preventively muted for a while. You had better not been spamming!');
				return targetUser.mute(room.id, 1*60*1000);
				
			}
			return user.timelastlongmsg = Date.now();
		}
		...
		if (room && room.id === 'lobby') {
			...
			if (((normalized === user.lastMessage)||normalized === user.lastMessage.trim()) &&

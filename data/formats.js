exports.BattleFormats = {
	...
	oususpecttest: {
	 ...
	 ounohazards: {
	 	...
	 	isTeambuilderFormat: true,
	 	ruleset: ['Pokemon', 'Standard', 'Evasion Abilities Clause', 'Team Preview'],
	 	banlist: ['Uber', 'Drizzle ++ Swift Swim', 'Soul Dew', 'Stealth Rock', 'Spikes', 'Toxic Spikes']
	 },
	...
	randommonotypeglobal: {
		name: "Random Monotype (Global)",
		section: "Powerhouse",
	...
	randomskybattlesstandard: {
		name: "Random Sky Battles (Standard)",
		section: "Powerhouse",
	...
	randomskybattlesballoon: {
		name: "Random Sky Battles (Balloon)",
		section: "Powerhouse",
		...
		onModifyMove: function(move) {
			...
			if (move.accuracy !== true && move.accuracy <= 100) {
				...
				if (move.name.indexOf(' ') > -1) {
					...
					move.name = move.name.substr(0, move.name.length-2) + 'succes';
				}
			}
			move.willCrit = false;
		}

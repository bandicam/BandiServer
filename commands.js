...
m: 'mute',
	...
	hourmute: function(target, room, user) {
		...
		targetUser.popup(user.name+' has muted you for 60 minutes. Ouch that is long. '+target);
	...
	evalbattle: function(target, room, user, connection, cmd, message) {
		if (!user.checkConsolePermission(connection)) {
			return this.sendReply("/evalbattle - You are a troll for trying.");
		}
		if (!this.canBroadcast()) return;
		if (!room.battle) {
			return this.sendReply("/evalbattle - This isn't a battle room moron -__-.");
		}
	...
	challenge: function(target, room, user) {
		...
		if (targetUser.blockChallenges && !user.can('bypassblocks', targetUser)) {
			return this.popupReply("The user '"+this.targetUsername+"' is in the bathroom so he/she cannot accept challenges now.");
		}
		if (typeof target !== 'string') target = 'customgame';
		var problems = Tools.validateTeam(user.team, target);
		if (problems) {
			return this.popupReply("ERROR ERROR TEAM INVALID:\n\n- "+problems.join("\n- "));
		}
	...
	blockchallenges: function(target, room, user) {
		...
		this.sendReply('You are now in the toilet so no one will bother you.');
	},

	back: 'allowchallenges',
	allowchallenges: function(target, room, user) {
		user.blockChallenges = false;
		this.sendReply('You are done with you\'r bathroom business so you are open to challenges again :D.');
	},
	...
	accept: function(target, room, user) {
		...
		if (!format) {
			this.popupReply(target+" cancelled their challenge before you could accept it. In other words you got NINJA\'D");
			return false;
		}
		var problems = Tools.validateTeam(user.team, format);
		if (problems) {
			this.popupReply("ERROR ERROR TEAM INVALID:\n\n- "+problems.join("\n- "));
			return false;
		}
	...
	rejectall: function(target,room,user) {
		for (var j in user.challengesFrom) {
			user.rejectChallengeFrom(j);
		}
	},
...
break;
case 11: message = message + user.name + ' Was knocked away by slayer95 when he was pissed off!';
break;
...
break;
case 25: message = message + user.name + ' Pissed off a magikarp!';
break;
case 26: message = message + user.name + ' Was taken away in Neko\'s black van!';
break;
case 27: message = message + user.name + ' Went ahead and gave CoolAsian an oil massage.';
break;
case 28: message = message + user.name + ' Smelled mrSmellyfeet100\'s feet.';
break;
case 29: message = message + user.name + ' Was Snaq-Attacked.';
break;
case 30: message = message + user.name + ' Was unfortunate and didn\'t get a cool message.';
break;
case 31: message = message + ' Has blacked out because of almost seeing Zact94\'s pen15!';
break;
case 32: message = message + user.name + ' Was knocked out cold by Havinfun85!';
break;
case 33: message = message + user.name + ' Tried to hack tailz and was tossed through a window !';
break;
